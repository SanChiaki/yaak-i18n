...
export function ResponseHeaders({ response }: Props) {
  ...
  return (
    <div className="overflow-auto h-full pb-4 gap-y-3 flex flex-col pr-0.5">
      <DetailsBanner
        storageKey={`${response.requestId}.general`}
        summary={<h2>{t("request:request.info")}</h2>}
      >
        <KeyValueRows>
          <KeyValueRow labelColor="secondary" label={t("request:response.requestUrl")}>
          ...
          <KeyValueRow labelColor="secondary" label={t("request:response.remoteAddress")}>
            {response.remoteAddr ?? <span className="text-text-subtlest">--</span>}
          </KeyValueRow>
          <KeyValueRow labelColor="secondary" label={t("request:response.version")}>
      ...
      <DetailsBanner
        ...
        summary={
          <h2 className="flex items-center">
            {t("request:response.requestHeaders")}{" "}
      ...
      <DetailsBanner
        ...
        summary={
          <h2 className="flex items-center">
            {t("request:response.responseHeaders")}{" "}
...
function NoHeaders() {
  const { t } = useTranslation();
  return (
    <span className="text-text-subtlest text-sm italic">{t("request:response.noHeaders")}</span>
  );
}
